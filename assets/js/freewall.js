...
(function ($) {
  ...
  var layoutManager = {
    ...
    loadBlock: function (item, setting) {
      ...
      $item.attr("data-height") == null &&
        $item.attr("data-height", $item.height());
      $item.attr("data-width") == null &&
      ...
      if (
        fixSize != null &&
        (col > runtime.limitCol || row > runtime.limitRow)
      ) {
        block = null;
      } else {
        ...
        if (fixPos) {
          ...
          block.x = 1 * fixPos[1];
          block.width =
            fixSize != null ? col : Math.min(col, runtime.limitCol - block.x);
          block.height =
            fixSize != null ? row : Math.min(row, runtime.limitRow - block.y);
          var holeId =
    ...
    showBlock: function (item, setting) {
      ...
      setting.delay > 0
        ? (item.delay = setTimeout(
            action,
            setting.delay * $item.attr("data-delay")
          ))
        : action();
    },
    ...
    setDraggable: function (item, option) {
      ...
      $(item).each(function () {
        ...
        $E.find("iframe, form, input, textarea, .ignore-drag").each(
          function () {
        ...
        $D.unbind("mouseup touchend", mouseUp);
        $D.unbind("mousemove touchmove", mouseMove);
        $H.unbind("mousedown touchstart").bind(
    ...
    setWallSize: function (runtime, container) {
      ...
      if (runtime.limitCol < runtime.limitRow) {
        ...
        !container.attr("data-height") &&
          container.height(Math.ceil(totalHeight));
      }
    },
  };
      ...
      for (var b = 0; b < bigLoop; ++b) {
        ...
        for (var s = 0; s < smallLoop; ++s) {
          ...
          if (block != null) {
            ...
            setting.onGapFound(
              layoutManager.setBlock(misBlock, setting),
              setting
            );
          }
        }
    ...
    $.extend(klass, {
      ...
      appendHoles: function (holes) {
        ...
        for (i = 0; i < newHoles.length; ++i) {
          h = newHoles[i];
          runtime.holes[h.top + "-" + h.left + "-" + h.width + "-" + h.height] =
            h;
        }
        return this;
      },
      ...
      fillHoles: function (holes) {
        if (arguments.length == 0) {
          ...
          for (i = 0; i < newHoles.length; ++i) {
            h = newHoles[i];
            delete runtime.holes[
              h.top + "-" + h.left + "-" + h.width + "-" + h.height
            ];
          }
        }
        return this;
      ...
      fixSize: function (option) {
        option.height != null &&
          $(option.block).attr({ "data-height": option.height });
        option.width != null &&
          $(option.block).attr({ "data-width": option.width });
        return this;
      },
